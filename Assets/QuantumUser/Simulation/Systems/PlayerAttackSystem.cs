...
namespace Quantum
{
    ...
    [Preserve]
    public unsafe class PlayerAttackSystem : SystemSignalsOnly, ISignalOnTrigger3D, ISignalOnTriggerEnter3D, ISignalOnTriggerExit3D, ISignalOnComponentAdded<AttackTargets>, ISignalOnComponentRemoved<AttackTargets>
    {
        public void OnTrigger3D(Frame f, TriggerInfo3D info)
        {
            ...
            var level = f.Get<PlayerEntityLevel>(info.Entity).DamageLevel;
            f.Signals.EntityHit(info.Other, playerConfig.Damage.Value(level) * f.DeltaTime);
        }
        ...
        private FP GetMaxDistanceEntity(FrameBase f, QList<EntityRef> list, FPVector3 playerPosition, out EntityRef entity)
        {
            var max = FP._0;
